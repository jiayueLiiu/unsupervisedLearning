...
class ResNetCIFAR(nn.Module):
    def __init__(self, num_layers=20, Nbits=None):
        ...
        self.g = nn.Sequential(nn.Linear(64, 128, bias=False), nn.BatchNorm1d(128),
                               nn.ReLU(inplace=True), nn.Linear(128, 128, bias=True))
    ...
    def forward(self, x):
        ...
        feature = torch.flatten(self.features, start_dim=1)
        self.out_unsep = self.g(self.feat_1d)
        return F.normalize(feature, dim=-1), F.normalize(self.out_unsep, dim=-1), self.final_fc_sup(self.feat_1d)
